...
Настроил интерфейс G0/0/0 для R2 на основе приведенной выше таблицы IP-адресации.
...
- Настроил интерфейс управления на S2 (VLAN 1), используя IP-адрес 192.168.1.98. Установил шлюз по умолчанию 192.168.1.97.
...
- На компьютере PC-A в настройках сетевой карты установил параметр "Получать данные автоматически". Затем открыл cmd и ввел команды ipconfig и ipconfig /renew для получения новой информации.
...
- На компьютере PC-B в настройках сетевой карты установил параметр "Получать данные автоматически". Затем открыл cmd и ввел команды ipconfig и ipconfig /renew для получения новой информации.
...
- После того, как я увидел, что компьютер получил адрес и связность в сети есть, повторно запустил на маршрутизаторе R1 команду show ip dhcp binding. Картина изменилась, в таблице появились IP-адреса выданные компьютерам.
...
Как видно из выводов по командам, на втором роутере таблица статистики пуста, так как он является всего лишь ретранслятором запросов.
